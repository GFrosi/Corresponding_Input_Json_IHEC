...
def create_dir(gse_list):
    '''receives a list of gse and creates 
    a directory using the gse ID
    '''
...
def get_srr(df, gse_list):
    ...
    including the GSE and SRR columns. Generates 
    txt files with a list of SRR per gse.
    Each file is saved in their respective 
    directories
    '''
...
def main():
    ...
    df = pd.read_csv(sys.argv[1]) #df with GSE and SRR columns
    gse_list = get_gse(df)
    create_dir(gse_list)
    get_srr(df, gse_list)
